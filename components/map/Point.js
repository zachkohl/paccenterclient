...
function Point(props) {
  ...
  const setter = useSelector((state) => {
    return {
      ...
      number: _.has(state, `points.${props.pointId}.number`)
        ? _.get(state, `points.${props.pointId}.number`)
        : "clear",
    };
  });
  if (setter.number === "clear") {
    return null;
  } else if (setter.walkinglist === null) {
    return (
      <Marker
